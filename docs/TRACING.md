...
```bash
cd deploy/dev/app
docker compose -p flashcode -f docker-compose-mid.yml up -d flashcode-skywalking-oap flashcode-skywalking-ui
```
...
ENV SW_AGENT_COLLECTOR_BACKEND_SERVICES="flashcode-skywalking-oap:11800"
...
flashcode 已集成 SkyWalking Logback 插件，日志会自动关联 TraceId。
...
```bash
# 查看 OAP 日志
docker logs flashcode-skywalking-oap
