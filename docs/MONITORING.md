...
flashcode 集成了 **Prometheus + Grafana + AlertManager** 作为服务监控与告警解决方案，提供：
...
```bash
cd deploy/dev/app
docker compose -p flashcode -f docker-compose-mid.yml up -d flashcode-prometheus flashcode-grafana flashcode-alertmanager
```
...
登录 Grafana 后，在左侧菜单选择 "Dashboards" → "flashcode"，可以看到：
...
4. 选择 "Prometheus"
5. 配置 URL：`http://flashcode-prometheus:9090`
6. 点击 "Save & Test"
...
```bash
# 在 Grafana 容器中测试连接
docker exec -it flashcode-grafana curl http://flashcode-prometheus:9090/-/healthy
```
...
3. 查看 AlertManager 日志：`docker logs flashcode-alertmanager`
...
2. 查看 AlertManager 日志：
   ```bash
   docker logs flashcode-alertmanager
   ```
